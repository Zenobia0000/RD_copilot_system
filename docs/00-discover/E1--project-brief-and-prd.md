...
- **結構化發散**：用 TRIZ（含 L1 跨域去錨定）擴大可能性空間，打破路徑依賴
...
>
> 下午他跑了 TRIZ L1 跨域去錨定，發現一條自己從沒想過的非同軸佈局方案。雖然直覺告訴他「不太可能」，但系統標出這條路線的假設和需要驗證的最小實驗——只要一次 thermal simulation 就能判斷。
>
...
|-------------|---------|---------------|---------------|------|--------|
| **G1: 擴大設計可能性空間** | 經驗鎖定、腦內庫存有限 | 方案探索路線數增加 | 方案探索數量（含跨域去錨定） | 1-2 條 | ≥3 條 |
...
| A6 | TRIZ L1 跨域去錨定能有效打破路徑依賴 | 工程師抗拒或流於形式 | Alpha 用戶觀察 |
...
| **US-03** | RD 工程師 | 啟動 TRIZ L1 跨域去錨定產生 ≥3 條反向路線 | 打破自己習慣的「安全牌」慣性 | 經驗鎖定、腦內庫存有限 | Feature 2 (跨域去錨定) |
...
#### UAT-03（對應 US-03）：跨域去錨定路線
- **Given** 至少一個 `confirmed` anchor solution
- **When** 使用者於 Explore 頁點 "Generate 跨域去錨定 Routes"
- **Then**：
