...
>
> **本文件來源**：從 `universal_template/01_GLOBAL_SYSTEM_PROMPT [TECH & CONSTRAINT LAYER]` + `02_PAGE_PROMPT.template.md` 元件 spec 提煉，依 Atomic Design 規格產出。
> **底層技術棧（universal_template 規範）**：React 18 + Tailwind CSS + Framer Motion + React Hook Form（表單）+ React Query（伺服器狀態）+ Zustand（全域狀態）+ React Table（表格）+ Recharts（圖表）。
...
| Color Picker | 顏色選擇器 |
| Slider / Range | 滑桿 |
| Stepper / Wizard | 步驟指示器（universal_template 多份 page spec 使用，建議升級為 P1） |
| Timeline | 時間軸（用於專案歷程） |
| Tree View | 樹狀結構（用於子系統階層） |
| Kanban Board | 看板（假設台帳 / 矛盾追蹤用） |
| Rich Text Editor | 富文字編輯器 |
| Code Block | 程式碼區塊（含語法高亮） |
| Chart Wrapper | 圖表容器（Recharts 為基底：Bar / Line / Pie / Radar） |
| Carousel | 輪播 |
...
## 5.5 RD Copilot 自訂業務元件（Domain Components）

> 從 `universal_template/02_PAGE_PROMPT.template.md` 各頁規格提煉，這些元件是 RD Design Copilot 業務語言的具現化。建議建立獨立 `features/` 目錄管理（universal_template 00_BLUEPRINT §7.1 專案結構規範）。

| 元件 | 用途 | 出現頁面 | Composition |
|------|------|---------|-------------|
| `KpiInputList` | 動態新增/刪除 KPI（名稱、目標值、衡量方式） | 任務定義 | Input × N + Add Button + Remove Button |
| `AiButton` | AI 觸發按鈕（含 sparkle icon + loading 動畫） | 多頁面通用 | Button + Loading Spinner + AI Icon |
| `SaveStatusIndicator` | 儲存狀態指示（idle / saving / saved） | 多頁面通用 | Icon + Text，淡入淡出切換 |
| `ContradictionCard` | 矛盾卡片（顯示 TC/PC/SF 類型 + 嚴重度 + 工程描述） | 矛盾識別、Explore | Card + Badge (severity) + Tag (type) |
| `AssumptionCard` | 假設卡片（驗證狀態 + 風險等級 + Owner + Due） | 假設台帳 | Card + Status Badge + Avatar + Date |
| `GateChecklistPanel` | Gate 退出條件檢查（分組、達成度進度條） | 各 Phase Gate 頁 | Accordion + Checkbox + Progress Bar |
| `EvidenceLink` | 證據連結（hover 顯示來源摘要 + 跳轉原文件） | 設計審查、證據鏈 | Link + Tooltip with metadata |
| `PreCadConfidenceGauge` | Pre-CAD 信心分數（綠/橘/紅環形圖 + 分數） | Pre-CAD Review | Circular Progress + Number + Label |
| `ContradictionConvergenceGraph` | 矛盾收斂 DAG 圖（節點 + 邊 + 健康警告） | 矛盾識別、儀表板 | React Flow / 自訂 SVG + Status indicators |
| `SocraticQuestionCard` | 蘇格拉底問答卡片（七類分組 + 標記為假設/矛盾） | Explore | Card + Category Badge + Action Buttons |
| `EntryGradingModal` | Entry Level A/B/C 判定 Modal | Explore 首次進入 | Modal + Radio Group + AI Reasoning |
| `LayeredSolutionCard` | 分層 TRIZ 解法卡片（L1/L2/L3 垂直堆疊） | 方案探索 / Create | Card stack with diff analysis |
| `CciBadge` | CCI 複雜度判定徽章（Evolution / Weak / Patch） | 方案決策中心 | Badge with score tooltip |
| `EvidenceCoverageGauge` | 證據覆蓋率（< 40% 橘色警告） | Create Decision Hub | Linear Progress + Threshold |
| `MissionContextPanel` | 任務背景面板（mission + constraints + KPIs + assumptions） | Create / Track | Collapsible Card with Sections |
| `ConstraintFeasibilityChecker` | 約束可行性檢查（Gate 1 必通過） | 任務定義 | Checklist + AI Validation |

**設計原則**：所有業務元件必須以 §3 P0 元件 + §4 P1 元件為基礎組合，不額外建立樣式系統。
...
**最後更新**：2026-04-28
**內容來源**：
- `universal_template/01_GLOBAL_SYSTEM_PROMPT.template.md` [TECH & CONSTRAINT LAYER] — 技術棧（React 18 + Tailwind + Framer Motion + React Hook Form + React Query + Zustand + React Table + Recharts）
- `universal_template/02_PAGE_PROMPT.template.md` 14 頁元件 spec — 業務元件清單（§5.5）

**相關文件**：[`00_foundations_spec.md`](./00_foundations_spec.md)、[`02_patterns_spec.md`](./02_patterns_spec.md)、[`03_templates_spec.md`](./03_templates_spec.md)、[`99_documentation_spec.md`](./99_documentation_spec.md)
