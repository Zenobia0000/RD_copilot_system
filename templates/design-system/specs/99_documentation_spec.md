...
> 這區是「工廠規章 + 變更公告」。
>
> **本文件來源**：§1-9 通用治理框架（v1.0 保留）+ 對應 `universal_template/04_QUALITY_CHECKLIST.template.md` 的 AI 產出驗證流程（§4 注入）。
> **AI 產出驗證**：當使用 universal_template 03_ASSEMBLY 將 Global + Page Prompt 餵給 AI 工具產出 React 程式碼或設計稿時，必須以 §4 Design QA Checklist + universal_template/04_QUALITY_CHECKLIST 雙線驗證。
...
## 10. universal_template Pipeline 整合

> 本節定義設計系統如何配合 `universal_template/` 的 Prompt Pipeline（00 Blueprint → 01 Global → 02 Page → 03 Assembly → 04 Quality）使用。

### 10.1 文件對應

| universal_template 階段 | 對應 design-system-specs | 用途 |
|:-----------------------|:------------------------|:-----|
| 00_FRONTEND_BLUEPRINT (架構 + IA) | `00_foundations` (tokens) + `03_templates` §11 (頁模板對照) | 提供 Foundation tokens + RD 14 頁模板對照 |
| 01_GLOBAL_SYSTEM_PROMPT | `00_foundations` + `02_patterns` (UX 規範) | 全域 prompt 從 §00, §02 萃取 |
| 02_PAGE_PROMPT × N | `01_components` §3-5 + `02_patterns` + `03_templates` | 每頁從 §01 抓元件、§02 抓模式、§03 抓模板 |
| 03_ASSEMBLY | 上述合併 | 給 AI 工具的最終 prompt |
| 04_QUALITY_CHECKLIST | `99_documentation` §4 | Design QA + AI 產出驗證 |

### 10.2 雙線品質驗證

當 AI 工具（Lovable / Claude / GPT-4 / v0）依照 03_ASSEMBLY 的 prompt 產出程式碼或設計稿，必須通過：

1. **第一線：universal_template/04_QUALITY_CHECKLIST**
   - 對齊 Global Prompt 中的具體值（color/font/breakpoint）
   - 檢查 AI 是否依三步驟回應（結構確認 → 設計決策 → 程式碼）

2. **第二線：本文件 §4 Design QA Checklist**
   - 核對 token 使用（不寫 magic number）
   - 核對所有狀態（loading/error/empty）
   - 核對 a11y（對比度、focus、tab 順序）
...
```
Foundation tokens 變更（00_foundations）
  ↓ 觸發
universal_template/01_GLOBAL_SYSTEM_PROMPT 更新
  ↓ 觸發
universal_template/03_ASSEMBLY 重組
  ↓ 觸發
所有頁面的 02_PAGE_PROMPT 重新 review

Pattern 變更（02_patterns §11.x RD 專用）
  ↓ 觸發
受影響的 02_PAGE_PROMPT 更新

頁模板變更（03_templates §11 業務頁映射）
  ↓ 觸發
受影響的 02_PAGE_PROMPT 更新
```
...
**版本**：v2.0（從 universal_template 提煉重構，新增 §10 universal_template Pipeline 整合）
**最後更新**：2026-04-28
**內容來源**：
- §1-9 通用治理框架 — 保留 v1.0 內容
- §10 universal_template Pipeline 整合 — 對應 `universal_template/04_QUALITY_CHECKLIST.template.md` + `03_ASSEMBLY.template.md`

**相關文件**：[`00_foundations_spec.md`](./00_foundations_spec.md)、[`01_components_spec.md`](./01_components_spec.md)、[`02_patterns_spec.md`](./02_patterns_spec.md)、[`03_templates_spec.md`](./03_templates_spec.md)、[`AI_DESIGN_INDUSTRIAL_PLAYBOOK.md`](./AI_DESIGN_INDUSTRIAL_PLAYBOOK.md)（meta-workflow）
